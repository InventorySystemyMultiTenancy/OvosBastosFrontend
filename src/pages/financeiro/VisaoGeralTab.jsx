...
import { gerarRelatorioVendas, gerarRelatorioClientesAtivos, gerarRelatorioEstoqueAtual, gerarRelatorioLucro } from '../../utils/relatoriosPdf';

function formatBRL(valor) {
  return Number(valor || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
}

const CONTA_RECEBER_VAZIA = { clienteId: '', valor: '', vencimento: '', caixaId: '', recorrente: false };
const CONTA_PAGAR_VAZIA = { descricao: '', fornecedor: '', valor: '', vencimento: '', caixaId: '', recorrente: false };
...
function proximosMeses(qtd) {
  ...
  return Array.from({ length: qtd }, (_, i) => {
    ...
    return { valor: mesParaChave(d), label: `${MESES_LABEL[d.getMonth()]}/${d.getFullYear()}` };
  });
}
...
export function VisaoGeralTab() {
  ...
  const meses = useMemo(() => proximosMeses(6), []);
  const [mesSelecionado, setMesSelecionado] = useState(() => mesParaChave(new Date()));
  ...
  const [modalRelatorioLucro, setModalRelatorioLucro] = useState(false);
  ...
  async function criarContaReceber(e) {
    ...
    await api.post('/financeiro/contas-receber', {
      ...
      recorrente: formReceber.recorrente,
    });
  ...
  async function pagarReceber(conta) {
    try {
      if (conta.virtual) {
        await api.post(`/financeiro/contas-receber/${conta.origemRecorrenteId}/receber-mes`, { mes: mesSelecionado });
      } else {
  ...
  async function cancelarRecorrenciaReceber(conta) {
    if (!confirm('Cancelar essa recorrência? Os meses já registrados continuam no histórico, só para de gerar meses novos.')) return;
    try {
      await api.put(`/financeiro/contas-receber/${conta.recorrenteTemplateId}/cancelar-recorrencia`, {});
      carregar();
    } catch (err) {
      alert(err.message);
    }
  ...
  async function criarContaPagar(e) {
    ...
    await api.post('/financeiro/contas-pagar', {
      ...
      recorrente: formPagar.recorrente,
    });
    setModalPagar(false);
  ...
  async function pagarConta(conta) {
    try {
      if (conta.virtual) {
        await api.post(`/financeiro/contas-pagar/${conta.origemRecorrenteId}/pagar-mes`, { mes: mesSelecionado });
      } else {
  ...
  async function cancelarRecorrenciaPagar(conta) {
    if (!confirm('Cancelar essa recorrência? Os meses já registrados continuam no histórico, só para de gerar meses novos.')) return;
    try {
      await api.put(`/financeiro/contas-pagar/${conta.recorrenteTemplateId}/cancelar-recorrencia`, {});
      carregar();
    } catch (err) {
      alert(err.message);
    }
  ...
  function abrirRelatorioLucro() {
    setPeriodoRelatorioLucro({ de: inicioDoMesISO(), ate: hojeISO() });
    setModalRelatorioLucro(true);
  }

  async function confirmarRelatorioLucro(e) {
    e.preventDefault();
    setGerando('lucro');
    try {
      const dados = await api.get(`/financeiro/relatorio-periodo?de=${periodoRelatorioLucro.de}&ate=${periodoRelatorioLucro.ate}`);
      await gerarRelatorioLucro(dados, periodoRelatorioLucro);
      setModalRelatorioLucro(false);
    } catch (err) {
      alert(err.message);
    } finally {
      setGerando(null);
  ...
  const RELATORIOS = [
    ...
    },
    {
      id: 'lucro',
      icone: '💰',
      titulo: 'Relatório de Lucro por Período',
      descricao: 'Tudo que foi pago no período escolhido, faturamento e o lucro que sobrou.',
      onClick: abrirRelatorioLucro,
    },
  ];
  ...
  const colunasReceber = [
    {
      ...
      render: (c) => (
        <span>
          {c.cliente.nome}
          {c.recorrenteTemplateId && <span className="badge badge-gray" style={{ marginLeft: 6 }}>🔁 Fixa mensal</span>}
        </span>
      ),
    },
    ...
    { key: 'pago', header: 'Status', render: (c) => <span className={`badge ${c.pago ? 'badge-green' : 'badge-amber'}`}>{c.pago ? 'Recebido' : 'Em aberto'}</span> },
    {
      ...
      render: (c) => (
        <div style={{ display: 'flex', gap: 8 }}>
          {!c.pago && <button className="btn btn-primary btn-sm" onClick={() => pagarReceber(c)}>Marcar recebido</button>}
          {c.recorrenteTemplateId && (
            <button className="btn btn-secondary btn-sm" onClick={() => cancelarRecorrenciaReceber(c)}>Cancelar recorrência</button>
          )}
        </div>
      ),
    },
  ...
  const colunasPagar = [
    ...
    { key: 'pago', header: 'Status', render: (c) => <span className={`badge ${c.pago ? 'badge-green' : 'badge-amber'}`}>{c.pago ? 'Pago' : 'Em aberto'}</span> },
    {
      ...
      render: (c) => (
        <div style={{ display: 'flex', gap: 8 }}>
          {!c.pago && <button className="btn btn-primary btn-sm" onClick={() => pagarConta(c)}>Marcar pago</button>}
          {c.recorrenteTemplateId && (
            <button className="btn btn-secondary btn-sm" onClick={() => cancelarRecorrenciaPagar(c)}>Cancelar recorrência</button>
          )}
        </div>
      ),
    },
  ...
  return (
    <div>
      ...
      <div className="section-title">Contas do mês</div>
      <div className="page-tabs">
        {meses.map((m) => (
          <button
            key={m.valor}
            type="button"
            className={`page-tab${mesSelecionado === m.valor ? ' is-active' : ''}`}
            onClick={() => setMesSelecionado(m.valor)}
          >
            {m.label}
          </button>
        ))}
      ...
      <Table columns={colunasReceber} rows={contasReceber} rowKey={(c) => c.id ?? `virtual-${c.origemRecorrenteId}`} />
      ...
      <Table columns={colunasPagar} rows={contasPagar} rowKey={(c) => c.id ?? `virtual-${c.origemRecorrenteId}`} />
          ...
          <form onSubmit={criarContaReceber}>
            ...
            <div className="field" style={{ marginTop: 4 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 400 }}>
                ...
                />
                Fixa mensal (repete todo mês a partir do vencimento)
              </label>
            </div>
      ...
      {modalRelatorioLucro && (
        <Modal title="Relatório de lucro por período" onClose={() => setModalRelatorioLucro(false)}>
          <form onSubmit={confirmarRelatorioLucro}>
            <p className="text-muted" style={{ marginBottom: 16 }}>Escolha o período — mostra tudo que foi pago e o lucro que sobrou (faturamento menos despesas).</p>
            <div className="form-grid">
              <div className="field">
                <label>De *</label>
                <input
                  type="date"
                  value={periodoRelatorioLucro.de}
                  onChange={(e) => setPeriodoRelatorioLucro({ ...periodoRelatorioLucro, de: e.target.value })}
                  required
                />
              </div>
              <div className="field">
                ...
                <input
                  type="date"
                  value={periodoRelatorioLucro.ate}
                  onChange={(e) => setPeriodoRelatorioLucro({ ...periodoRelatorioLucro, ate: e.target.value })}
                  required
                />
              </div>
            </div>
            <div className="modal-actions">
              <button type="button" className="btn btn-secondary" onClick={() => setModalRelatorioLucro(false)}>Cancelar</button>
              <button type="submit" className="btn btn-primary" disabled={gerando === 'lucro'}>
                {gerando === 'lucro' ? 'Gerando...' : '📄 Gerar PDF'}
              </button>
            </div>
          </form>
        </Modal>
